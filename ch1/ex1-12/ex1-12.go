/*
Modify the Lissajous server to read parameter values from the URL.
For example, you might arrange it so that a URL like 
http://localhost:8000/?cycles=20 sets the number of cyles to 20
instead of the default 5.

Use the 'strconv.Atoi' function to convert the string parameter 
into an integer. You cna see its documentation with 
	`go doc strcov.Atoi`   
*/
package main

import (
	"image"
	"image/color"
	"image/gif"
	"io"
	"log"
	"math"
	"math/rand"
	"net/http"
)


var palette = []color.Color{color.White, color.Black}

const (
	whiteIndex = 0 // first color in palette
	blackIndex = 1 // next color in palette
)

func main() {
	
	rand.Seed(time.Now().UTC().UnixNano())

	http.HandleFunc("/", handler)
	log.Fatal(http.ListenAndServe("localhost:8000", nil))

}

func handler(w http.ResponseWriter, r *http.Request) {
	params := map[string]int {
		"cycles":	5,
		"size":		100,
		"nframes":	64,
		"delay":	8
	}

	for param := range params {
		if r.FormValue(param) != "" {
			params[param], _ = strconv.Atoi(r.FormValue(param))
		}
	}

	lissajous(w, params)
}

func lissajous(out io.Writer) {
	cycles := params["cycles"]
	res:= 0.001
	size := params["size"]
	nframes := params["nframes"]
	delay := param["delay"]
	
	freq := rand.Float64() * 3.0 // relative frequency of y oscillator
	anim := gif.GIF{LoopCount: nframes}
	phase := 0.0 // phase difference
	for i := 0; i < nframes; i++ {
		rect := image.Rect(0, 0, 2*size+1, 2*size+1)
		img := image.NewPaletted(rect, palette)
		for t := 0.0; t < cycles*2*math.Pi; t += res {
			x := math.Sin(t)
			y := math.Sin(t*freq + phase)
			img.SetColorIndex(size+int(x*size+0.5), size+int(y*size+0.5),
				blackIndex)
		}
		phase += 0.1
		anim.Delay = append(anim.Delay, delay)
		anim.Image = append(anim.Image, img)
	}
	gif.EncodeAll(out, &anim) // NOTE: ignoring encoding errors
}

//!-main
